...
type DeviceDeploymentSpec struct {
	...
	// Canary defines a new alternative template of the pod that will be created for weighted group of devices
	Canary *CanarySpec `json:"canary,omitempty"`
}

// CanarySpec defines the desired state of the canary part of a DeviceDeployment
type CanarySpec struct {
	// Percentage (0-100) defines the number of devices of which the canary template should be considered over the normal template
...
type Selector struct {
	...
	// +optional
	MatchByLabels map[string]string `json:"matchByLabels,omitempty"`
