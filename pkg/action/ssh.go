...
func SSH(s *service.Service, id string, flagCommand string, silent bool) error {
...
func createSession(conf *SSHConfig) error {
	...
	go func() {
		for {
			...
			case api.ProcessOutputMessageType:
				var output api.ProcessOutputMessage
			...
			case api.WriteToFileMessageType:
				if conf.OutputWG != nil {
					conf.OutputWG.Add(1)
				}
				conf.OutputChannel <- message
			case api.ProcessCreatedMessageType:
				connectedChannel <- 0

				var created api.ProcessCreatedMessage
			...
			case api.ProcessTerminatedMessageType:
				exitChannel <- 0
				return
			case api.ErrorMessageType:
